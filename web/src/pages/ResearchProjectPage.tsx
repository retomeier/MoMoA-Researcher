    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        for (const pastSession of pastSessions) {
          // 1. Get initial file uploads provided by the user in past sessions
          if (pastSession.actionQueue) {
            const actions = Object.values(pastSession.actionQueue) as IncomingAction[];
            const fileChunk = actions.find(a => a.status === "FILE_CHUNK");
          ...
          // 2. Get the modified files generated by the agent at the end of past sessions
          if (pastSession.history) {
            const historyItems = Object.values(pastSession.history) as HistoryItem[];
            const completeItem = historyItems.find((h) => h.status === "COMPLETE_RESULT");
            if (completeItem?.data?.files) {
              try {
                const parsedFiles = JSON.parse(completeItem.data.files);
