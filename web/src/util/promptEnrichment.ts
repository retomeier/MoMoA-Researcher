...
import { SESSION_ROOT_PATH, HistoryItem } from "../../../src/shared/model";
import { ProjectMetadata } from '../document/model-and-db';
import { decode as b64decode } from "js-base64";

const ResearchEnrichment: string = 
...
* Step 1: Review the raw data and create a detailed outline based on the Required Structure below.
* Step 2: Loop through the outline, creating a new Work Phase task to write each section at a time.
...
export function getEnrichedResearchPrompt(initialProjectPrompt: string): string {
  const enrichedString = ResearchEnrichment.replace("${ResearchProblem}", initialProjectPrompt);
  return enrichedString;
}

/**
 * Helper to determine if a string is likely binary or text.
 */
function isBinary(content: string): boolean {
  // Check for common binary patterns or null bytes after decoding
...
/**
 * Gathers project and session history plus the latest file state.
 */
export async function buildProjectContextPrompt(
  projectId: string,
  sessionId: string,
  ...
  // 1. Get original project definition
  const projectDefinition = project.initialPrompt || project.title || "No overarching project definition provided.";

  // 2. Fetch past sessions context & Accumulate Files
  ...
  if (snapshot.exists()) {
    ...
    snapshot.forEach((childSnap) => {
      ...
      if (childSnap.key !== sessionId && isActuallyComplete) {
    ...
    // Build history string AND accumulate latest files
    pastTasksContext = pastSessions.map((session, index) => {
       const hItems = Object.values(session.history || {}) as HistoryItem[];
       
       // Accumulate files from history results
       const completeItem = hItems.find(item => item.status === "COMPLETE_RESULT");
       if (completeItem?.data?.files) {
         try {
           const parsed = JSON.parse(completeItem.data.files);
           parsed.forEach((f: any) => {
             if (!EXCLUDED_FILES.has(f.name.toLowerCase())) accumulatedFiles.set(f.name, f.content);
           });
         } catch(e) {}
       }

       // Accumulate files from initial uploads in those sessions
       ...
       const promptItem = hItems.find(item => item.status === "USER_MESSAGE");
       return `**Research Task ${index + 1}:**:\nAssigned Task:\n${promptItem?.message || "N/A"}\nTask Result Summary:\n${completeItem?.data?.retrospective || "N/A"}`;
    }).join("\n\n");
  }

  // 3. Process Files for Context (Apply Size and Binary Constraints)
  const fileContextItems: string[] = [];
  accumulatedFiles.forEach((b64Content, fileName) => {
    const rawContent = b64decode(b64Content);
    const sizeInBytes = rawContent.length;

    if (isBinary(b64Content)) {
      const extension = fileName.split('.').pop()?.toUpperCase() || "Unknown";
      fileContextItems.push(`File: ${fileName} (Binary ${extension} file - Content omitted)`);
    } else if (sizeInBytes > 100000) {
      fileContextItems.push(`File: ${fileName} (This file is too large to include in context: ${Math.round(sizeInBytes/1024)}kb)`);
    } else {
      fileContextItems.push(`File: ${fileName}\nContent:\n\`\`\`\n${rawContent}\n\`\`\``);
    }
  });

  const filesContextString = fileContextItems.length > 0 
    ? `### Current Project Files (Newest Versions):\n${fileContextItems.join("\n\n")}`
    : "No new or modified files in project.";

  // 4. Extract Current Session History
  const historyRef = ref(db, `${SESSION_ROOT_PATH}/${sessionId}/history`);
  const historySnap = await get(historyRef);
  let currentPrompt = "No initial prompt found.";
  let currentLogText = "";

  if (historySnap.exists()) {
    const currentSessionHistoryItems = Object.values(historySnap.val()) as HistoryItem[];
    const currentPromptItem = currentSessionHistoryItems.find(item => item.status === "USER_MESSAGE");
    currentPrompt = currentPromptItem?.message || "No initial prompt found.";

    currentLogText = currentSessionHistoryItems.flatMap(item => {
      switch (item.status) {
        case "USER_MESSAGE":
        ...
        case "PROGRESS_UPDATES": {
          if (item.completed_status_message) {
            const  text = `${item.completed_status_message}`;
            //if (item.data?.result) text += `\n\n----\n\nFinal Result Summary: ${item.data.result}`;
            return [text];
          }
        }
      }
  ...
  const enrichedProjectDefinition = getEnrichedResearchPrompt(projectDefinition);
...
### Project Definition
""""
${enrichedProjectDefinition}
""""

### Past Research History
""""
${pastTasksContext || "No previous tasks completed yet."}
""""
